...
#Campaign Stats
df_engr$campaign_duration = round((db_cleaned$deadline - db_cleaned$launched_at), 2)
...
df_engr$description_length[is.na(db_cleaned$description_length)] <- 0

#Dates
df_engr$date_launched <- as.Date(db_cleaned$launched_at, format="%m/%d/%Y")
df_engr$mo_yr_launched <- format(db_cleaned$date_launched, format="%Y-%m")
df_engr$year_launched <- format(db_cleaned$date_launched, format="%Y")
```
