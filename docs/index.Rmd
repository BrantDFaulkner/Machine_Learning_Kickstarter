...
#Brant Adding things
db_cleaned$US = as.numeric(db_cleaned$country == "US")
db_cleaned$funded <- db_cleaned$state
db_cleaned$facebook <- as.numeric(!is.na(db_cleaned$facebook)) 
db_cleaned$twitter <- as.numeric(!is.na(db_cleaned$twitter)) 
db_cleaned$youtube <- as.numeric(!is.na(db_cleaned$youtube))
db_cleaned$social_media_count <- db_cleaned$facebook + db_cleaned$twitter + db_cleaned$youtube
db_cleaned$description_length <- replace_na(db_cleaned$description_length, 0)
...
set.seed(1)
n <- nrow(db_cleaned)
shuffled_df <- db_cleaned[sample(n), ]
...
db_cleaned <- db_cleaned %>% 
  mutate(backers_count_20 = cut(backers_count, breaks = unique(quantile(backers_count, seq(0, 1, by = .05))), include.lowest = TRUE))
...
lm <- lm(funded ~ US, data = db_cleaned)
summary(lm)
```
...
## Explore $photo_key
There are not enough zero values for prediction.
```{r}
db_cleaned %>%
  group_by(photo_key) %>%
    summarise(n(), mean(funded))
...
lm <- lm(funded ~ social_media, data = db_cleaned)
summary(lm)
...
lm <- lm(funded ~ social_media_count, data = db_cleaned)
summary(lm)
